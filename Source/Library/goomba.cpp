...
{
	void Goomba::UpData(Goomba goomba, Mario mario,Map map)
	{
		Collision(goomba,map);
		if (isCollision == true)
		{
			horizontalSpeed *= -1;
			isCollision = false;
	...
	void Goomba::Collision(Goomba goomba,Map map)
	{
		vector<vector<int>> map_vector = map.GetMap();
		int mario_y = goomba.GetTop() / 64;

		//left collision
		if (horizontalSpeed > 0)
		{
			int mario_x = (goomba.GetLeft() - map.GetLeft()) / 64;
